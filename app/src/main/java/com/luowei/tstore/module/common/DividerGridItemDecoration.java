...
public class DividerGridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private boolean isFirstColum(int itemPosition, RecyclerView parent) {
        if (itemPosition % getSpanCount(parent) == 0) {
            return true;
        }
        return false;
    }

    private boolean isFirstRaw(int itemPosition, RecyclerView parent) {
        if (itemPosition < getSpanCount(parent)) {
            return true;
        }
        return false;
    }
    ...
    public void getItemOffsets(Rect outRect, int itemPosition,
        ...
        }else if (isFirstColum(itemPosition, parent)) {
            outRect.set(mDivider.getIntrinsicWidth(), 0, mDivider.getIntrinsicWidth(),
                    mDivider.getIntrinsicHeight());
        } else if (isFirstRaw(itemPosition, parent)) {
